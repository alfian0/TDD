...
#Preview {
	CountrySearchListView(viewModel: CountryCodeViewModel(selected: .dummy, items: try! Data.fromJSONFile("Dial").toCodable(with: [CountryCodeResponse].self).map({ CountryCodeModel(name: $0.name, flag: $0.flag, dialCode: $0.dialCode, code: $0.code)})) { item in
